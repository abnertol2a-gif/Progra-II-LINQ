...
namespace GestionAcademicaLINQ
{
    class Program
    {
        static void Main()
        {
            ...
            string rutaSalida = "C:\\Users\\Abner\\OneDrive\\Documents\\Visual Studio 2022\\Progra 2-linq\\Progra-II-LINQ\\Ejercicio3-LINQ\\Ejercicio3-LINQ\\resultados.txt";

            List<Estudiante> estudiantes = CargarEstudiantes(rutaArchivo);

            var aprobados = estudiantes.Where(e => e.Nota >= 70);
            var top5PorCurso = estudiantes.GroupBy(e => e.Curso).SelectMany(g => g.OrderByDescending(e => e.Nota).Take(5));
            var promedioPorCurso = estudiantes.GroupBy(e => e.Curso).Select(g => new { Curso = g.Key, Promedio = g.Average(e => e.Nota) });
            var top10General = estudiantes.OrderByDescending(e => e.Nota).Take(10);
            var ranking = estudiantes.OrderByDescending(e => e.Nota).Select((e, i) => new { Posicion = i + 1, Estudiante = e });
            var mejorPorCurso = estudiantes.GroupBy(e => e.Curso).Select(g => g.OrderByDescending(e => e.Nota).First());
            ...
            {
                sw.WriteLine("=== Estudiantes Aprobados ===");
                foreach (var e in aprobados) sw.WriteLine(e);

                sw.WriteLine("\n=== Top 5 por Curso ===");
                foreach (var e in top5PorCurso) sw.WriteLine(e);

                sw.WriteLine("\n=== Promedio por Curso ===");
                foreach (var p in promedioPorCurso) sw.WriteLine($"{p.Curso}: {p.Promedio:F2}");

                sw.WriteLine("\n=== Top 10 General ===");
                foreach (var e in top10General) sw.WriteLine(e);

                sw.WriteLine("\n=== Ranking General ===");
                foreach (var r in ranking) sw.WriteLine($"#{r.Posicion} - {r.Estudiante.Nombre} ({r.Estudiante.Nota})");

                sw.WriteLine("\n=== Mejor Estudiante por Curso ===");
                foreach (var e in mejorPorCurso) sw.WriteLine($"{e.Curso}: {e.Nombre} ({e.Nota})");

                sw.WriteLine("\n=== Intervalos de Notas ===");
                sw.WriteLine("0-59:");
                foreach (var e in intervalo0_59) sw.WriteLine(e);
                sw.WriteLine("\n60-79:");
                foreach (var e in intervalo60_79) sw.WriteLine(e);
                sw.WriteLine("\n80-100:");
                foreach (var e in intervalo80_100) sw.WriteLine(e);
            }
